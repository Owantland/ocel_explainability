"""
Experiment: does num_layers=5 close the gap to HomoGNN on TimeFrom_CustomerOrder_to_Depart's
last-event predictions further than num_layers=3 already did? num_layers=3 (MAE_last=28.2h,
R2_last=-23.6) already closed most of the gap from the num_layers=2 baseline (MAE_last=243.6h,
R2_last=-1370.9) by giving HGT one more hop to route Events signal into the CustomerOrder
(viewpoint) node. A past paper on this dataset used num_layers=5 -- testing whether more depth
helps further, or whether returns have diminished / started reversing (over-smoothing).

Same hidden_channels/num_heads/lr as the sweep's best trial, weight_decay=0 (the confirmed
lin_dict fix), no loss reweighting -- directly comparable to both prior experiments.
Saves to a separate checkpoint path, doesn't overwrite any prior variant.
"""
import copy
import json
import time
...
DATABASE = 'logistics'
CANT = 1000
CHECKPOINT_PATH = 'files/models/logistics/1000/Hetero/TimeFrom_CustomerOrder_to_Depart_layers5_experiment.pth'
...
with open(CHECKPOINT_PATH.replace(".pth", "_norm.json"), "w") as f:
    json.dump({"target_mean": m.target_mean.item(), "target_std": m.target_std.item(),
               "fit_time_s": fit_time_s, "num_layers": 5}, f)
