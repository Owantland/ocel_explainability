...
| Epochs / early stopping | 30 / patience 4 — **used uniformly** for tuning and final results | Sweep: 30 epochs, patience 4 (both datasets). **Final training: 30/4 for `order_management`, 100/10 for `logistics`** as of 2026-07-09 (dataset-conditional in `self.max_epochs`/`self.early_stop_patience`, `training.py`'s `Modelling.__init__`) — see recommendation 1 for why the two datasets diverge. |
| Optimizer | Adam | Adam |
...
**Resolved 2026-07-09, then refined same day** (see recommendation 1): the epoch/patience mismatch was the most material finding here — HOEG deliberately uses **one** small budget everywhere so tuning and reported results are directly comparable, but `Het_Reg_Modelling`/`Homo_Reg_Modelling` previously trained for over 3x longer with 2.5x the patience versus `sweep()`, as an undocumented accident of two numbers living in different places rather than a stated methodological choice. First attempt: aligned both datasets down to 30/4 to match `sweep()` and HOEG's Table 5/7 exactly. Retraining `order_management`/PayOrder under this budget worked well (last-event MAE improved 102.7h→95.9h, R² 0.097→0.197, and HGT now beats HomoGNN on last-event where it previously lost narrowly). Retraining `logistics`/`CustomerOrder_to_Depart` under the same 30/4 budget **failed badly**: early stopping fired at epoch 6, and the model collapsed to a near-constant predictor (last-event R²=-1700.6 vs. HomoGNN's well-behaved 6.2h MAE; feature attribution came back exactly zero for every node type except the viewpoint). Root cause: `logistics`'s last-event class is far rarer relative to its dataset (138 of 8651 test prefixes) than `order_management`'s, and needs materially more epochs to learn anything about it before patience=4 kills training. **Resolution**: kept `order_management` at 30/4 (matches HOEG, works well), reverted `logistics` specifically to the pre-existing 100/10 budget (dataset-conditional in `training.py`'s `Modelling.__init__`) — a deliberate, documented deviation from strict HOEG parity for this one dataset, not an oversight. `logistics`/`CustomerOrder_to_Depart` needs retraining again under 100/10 to produce a usable checkpoint; `order_management`/PayOrder's 30/4 retrain is already good and doesn't need to change again.
...
1. ✅ **[Implemented, 2026-07-09, refined same day]** Reconciled the epoch/patience mismatch — `Het_Reg_Modelling`'s (and `Homo_Reg_Modelling`'s) final-training budget matches `sweep()`'s (30/4) for `order_management`, restoring direct comparability with HOEG's Table 5/7 single-budget methodology there. `logistics` deliberately kept on the longer 100/10 budget instead (dataset-conditional in `Modelling.__init__`), since 30/4 caused it to collapse to a near-constant predictor — its last-event class is too rare (138/8651 test prefixes) to learn under such aggressive early stopping. This is a documented, justified per-dataset deviation, not an oversight.
...
5. **Use HOEG's Section 6.1 findings as an informed prior for the Optuna search — updated 2026-07-04, now split into concrete, curtailed-search-space steps**, since seeding priors alone (already implemented via `enqueue_trial`) doesn't stop the sweep from still wandering back to a worse, unreachability-inducing choice:
   ...
   - **5g. [Implemented, 2026-07-09] Second, empirically-forced deviation from HOEG for `logistics`: `num_layers` is now tuned in `{3,4,5}` instead of fixed at 2.** Retraining `TimeFrom_CustomerOrder_to_Depart` under the fixed depth=2 collapsed the model to a near-constant predictor (last-event R²≈-1700) regardless of which epoch/patience budget was used (see recommendation 1) — ruling out training duration as the cause. A prior one-off experiment (`experiment_num_layers3.py`, 2026-07-07) already showed `num_layers=3` alone cuts last-event MAE from 243.6h/R²=-1370.9 (layers=2) to 28.2h/R²=-23.6 — a large but incomplete fix, hence testing up to 5. `order_management` is unaffected — still fixes `num_layers=2` per 5a, since that path works well. `logistics`'s grid grows to 6 hidden × 2 lr × 3 layers = 36 combinations (from 12) as a result. `sweep()` also gained SQLite-backed persistent storage (`storage=`/`study_name=`/`load_if_exists=True`) for both datasets so a killed/interrupted run — which has happened on long `logistics` sweeps — can resume via a plain re-call of `sweep()` instead of losing all trial progress.
