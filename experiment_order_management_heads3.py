"""
Experiment: does num_heads=3 improve TimeFrom_Orders_to_PayOrder on order_management,
keeping num_layers=2 (the currently-active architecture) and weight_decay=0.0 (the
currently-active regularization)? HGTConv requires hidden_channels % num_heads == 0,
and the official hidden_channels=64 is not divisible by 3, so hidden_channels is bumped
to 63 (closest multiple of 3) -- everything else matches the official params (64->63/2/0.001/0.0).

Saves to a separate checkpoint path, does NOT overwrite the official
TimeFrom_Orders_to_PayOrder.pth.
"""
import copy
import json
import time
...
CANT = 2000
CHECKPOINT_PATH = 'files/models/order_management/2000/Hetero/TimeFrom_Orders_to_PayOrder_heads3_experiment.pth'

m = t.Modelling(DATABASE, CANT)
params = dict(m.params)
print(f"Baseline params (currently active, num_heads=2, hidden_channels=64): {params}")
params['num_heads'] = 3
params['hidden_channels'] = 63
...
with open(CHECKPOINT_PATH.replace(".pth", "_norm.json"), "w") as f:
    json.dump({"target_mean": m.target_mean.item(), "target_std": m.target_std.item(),
               "fit_time_s": fit_time_s, "num_heads": 3, "hidden_channels": 63}, f)
