"""
Experiment: does num_layers=3 (one more hop) close the gap to HomoGNN on
TimeFrom_CustomerOrder_to_Depart's last-event predictions? HomoGNN reads its prediction
from global_mean_pool over ALL Events nodes; HGT reads only the CustomerOrder (viewpoint)
node's own embedding after num_layers hops of attention-weighted message-passing. If 2 hops
isn't enough to route Events signal into CustomerOrder effectively, a 3rd hop might help.

Isolates the num_layers effect specifically: same hidden_channels/num_heads/lr as the sweep's
best trial, weight_decay=0 (the already-confirmed lin_dict fix), no loss reweighting -- directly
comparable to the "weight_decay=0" experiment's results (MAE_last=243.6h, R2_last=-1370.9).
...
DATABASE = 'logistics'
CANT = 1000
CHECKPOINT_PATH = 'files/models/logistics/1000/Hetero/TimeFrom_CustomerOrder_to_Depart_layers3_experiment.pth'
