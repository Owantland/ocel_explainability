"""
Full pipeline for cant=2000, order_management.
Assumes OCEL (ocel.csv, edges.csv, ev_log.csv, all_kpis.csv) already exists.
Steps: train-test split → hetero graphs → sweep → train → validate.
"""
import torch
import numpy as np
import pandas as pd
...
# ── 1 & 2. Graphs already built — skip to sweep ───────────────────────────────

# ── 3. Training with saved hyperparameters (skip sweep — params already saved) ─
# model_params.json has: hidden=48, layers=3, heads=2, lr=0.000149
print("Training with saved hyperparameters...")
...
print(f"TRACE EXPLANATION — fast order (id={fast_id}, true={fast_true:.0f}h)")
print("=" * 60)
e.explain_trace(fast_id, top_k=5)

print("\n" + "=" * 60)
slow_true = last_sorted[last_sorted['order_id'] == slow_id]['true_h'].values[0]
print(f"TRACE EXPLANATION — slow order (id={slow_id}, true={slow_true:.0f}h)")
print("=" * 60)
