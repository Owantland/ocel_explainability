...
by (database, cant, mode, order_id[, n_events][, top_k]), storing exactly what the
...
def cache_path(database, cant, mode, order_id, n_events=None, top_k=5):
    ...
    return os.path.join(CACHE_ROOT, f"{database}_{cant}", mode, f"{order_id}{ev_suffix}{k_suffix}.json")


def load_cached(database, cant, mode, order_id, n_events=None, top_k=5):
    path = cache_path(database, cant, mode, order_id, n_events, top_k)
...
def save_cached(database, cant, mode, order_id, result, n_events=None, top_k=5):
    path = cache_path(database, cant, mode, order_id, n_events, top_k)
...
def get_or_compute(database, cant, mode, order_id, compute_fn, n_events=None, top_k=5):
    ...
    cached = load_cached(database, cant, mode, order_id, n_events, top_k)
    if cached is not None:
        return cached, True
    result = compute_fn()
    save_cached(database, cant, mode, order_id, result, n_events, top_k)
    return result, False
