"""
Experiment: does combining num_layers=3 AND weight_decay=0 improve TimeFrom_Orders_to_PayOrder
further than either fix alone? Prior results on this model:
  - num_layers=2, weight_decay=1e-5 (original)            : MAE_last=102.7h, R2_last=0.097
  - num_layers=3, weight_decay=1e-5                        : MAE_last=104.5h, R2_last=0.081 (no help)
  - num_layers=2, weight_decay=0.0 (currently promoted)    : MAE_last=88.7h,  R2_last=0.267 (helped)
This tests num_layers=3, weight_decay=0.0 together.

Same hidden_channels/num_heads/lr as the currently-saved params (64/2/0.001) otherwise.
Saves to a separate checkpoint path, does NOT overwrite the official TimeFrom_Orders_to_PayOrder.pth.
"""
import copy
import json
import time
...
CANT = 2000
CHECKPOINT_PATH = 'files/models/order_management/2000/Hetero/TimeFrom_Orders_to_PayOrder_layers3_wd0_experiment.pth'

m = t.Modelling(DATABASE, CANT)
params = dict(m.params)
print(f"Baseline params (currently active, layers=2/weight_decay=0): {params}")
params['num_layers'] = 3
...
with open(CHECKPOINT_PATH.replace(".pth", "_norm.json"), "w") as f:
    json.dump({"target_mean": m.target_mean.item(), "target_std": m.target_std.item(),
               "fit_time_s": fit_time_s, "num_layers": 3, "weight_decay": 0.0}, f)
