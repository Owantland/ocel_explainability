...
- **`train_test_builder.py` (`TrainTestBuilder`)** — builds temporally-non-overlapping train/val/test id splits. Note: `get_active_orders` trims and rewrites `ev_log.csv` **in place, once per viewpoint object**, inside a loop — an O(n) sequence of full-file rewrites rather than a single batch write; a performance smell and a correctness risk if interrupted mid-run.
...
- **The `db_testing.py` validation suite is dataset-schema-specific but not dataset-parameterized in practice.** `verify_ocel_generator`, `verify_hetero_graphs`, and `compare_to_hoeg` are hardcoded to `order_management` column names (`Items::ids`, `object_Employees`, etc.) and are explicitly commented out for `logistics` in the module's own main block because they crash. Fix: either generalize the checks to read object types from `config.yml` dynamically, or split per-dataset verification functions and stop pretending they're dataset-agnostic.
