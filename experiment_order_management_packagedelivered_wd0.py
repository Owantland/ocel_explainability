"""
Experiment: does removing weight_decay (0.0 instead of 1e-5) improve TimeFrom_Orders_to_PackageDelivered
on order_management, keeping num_layers=2 (the currently-active architecture)? Unlike
CustomerOrder->Depart on logistics, this model's lin_dict was already fully healthy at
weight_decay=1e-5 (no dead projections) -- so this isn't expected to fix a structural problem
the way it did there, but tests whether removing regularization entirely still helps or hurts
an already-working model.

Same hidden_channels/num_heads/lr as the currently-saved params (64/2/0.001) -- isolates the
weight_decay effect specifically. Saves to a separate checkpoint path, does NOT overwrite the
official TimeFrom_Orders_to_PackageDelivered.pth (already backed up separately).
"""
import copy
import json
import time
...
CANT = 2000
CHECKPOINT_PATH = 'files/models/order_management/2000/Hetero/TimeFrom_Orders_to_PackageDelivered_wd0_experiment.pth'

m = t.Modelling(DATABASE, CANT)
params = dict(m.params)
print(f"Baseline params (currently active, weight_decay=1e-5): {params}")
...
with open(CHECKPOINT_PATH.replace(".pth", "_norm.json"), "w") as f:
    json.dump({"target_mean": m.target_mean.item(), "target_std": m.target_std.item(),
               "fit_time_s": fit_time_s, "weight_decay": 0.0}, f)
